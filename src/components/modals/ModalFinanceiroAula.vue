...
<template>
  <div class="modal-backdrop-custom" v-if="ui.modals.financeiro && aulasStore.aulaFinanceiro" @click.self="ui.modals.financeiro = false">
    <div class="modal-sheet" v-if="aulasStore.aulaFinanceiro">
      <div class="modal-handle"></div>
      <div class="modal-title"><i class="bi bi-pie-chart-fill"></i> Divisão Financeira</div>
      <div style="font-size:.8rem;color:var(--text-muted);margin-bottom:6px">{{ formatDate(aulasStore.aulaFinanceiro.data) }} — {{ catalog.getNucleoNome(aulasStore.aulaFinanceiro.nucleoId) }}</div>
      ...
      <div class="card p-0 mb-3" style="border-radius:0 0 8px 8px">
        <div v-for="aa in aulasStore.aulaFinanceiro.alunos.filter(a => a.presente)" :key="aa.alunoId" style="display:flex;justify-content:space-between;align-items:center;padding:9px 14px;border-bottom:1px solid var(--border);font-size:.85rem">
          <div>
        ...
        <div v-if="!aulasStore.aulaFinanceiro.alunos.some(a => a.presente)" style="padding:12px 14px;font-size:.8rem;color:var(--text-muted);text-align:center">Nenhum aluno presente.</div>
      </div>
